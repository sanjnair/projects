 ...
 */
bool AdjMatrix::getValue( int row, int col ) const {
  if( (row< _dim) && (col < _dim) ) {
    return( _adjMatrix[row][col]);
  }
  return false;
}
 ...
 * returns the weblist
 */
WebList AdjMatrix::getWebList() const {
  return _webList;
}
 ...
 * builds the adjacency matrix
 */
void AdjMatrix::build( const ConstraintVector &vct ) {
 ...
 * interference
 */
void AdjMatrix::buildRegisterInterference(const ConstraintVector &vct) {
  int nRegs = RegSpec::getInstance()->getNumMachineRegs();
  for(int i=0; i<nRegs;i++) {
    if( hasConstraints(i+1, vct) ) { //register number starts with 1
      interferenceWithConstraints(i+1, vct);
    }else {
      interferenceWithNoConstraints(i+1, vct);
    }
  }
}
 ...
 * interference.
 */
void AdjMatrix::buildSpillInterference() {
  int sRegIndex = -1;
  int mRegIndex = -1;
  IntList list;
  ...
  ConstraintVector *newVct = new ConstraintVector();
  for( ConstraintVector::const_iterator it = vct.begin();
       it != vct.end();
       it ++ ) {

    //loop through the weblist to search for the register
    //sRegIndex = _sRegIndexMap[it->_symReg];
    mRegIndex = _mRegIndexMap[it->_machineReg];
    
    list.clear();
    for( WebList::const_iterator listIt = _webList.begin();
         listIt != _webList.end(); listIt++ ) {

      //if the register constraint and the web register 
      //matches, then we'll do the machine register
      //constraints.


      if( listIt->getReg()->num == it->_symReg ) {
        sRegIndex = _sRegIndexMap[listIt->getWebReg()->num];
        if( ! Vertex::isItemInList(list, sRegIndex) ) {
          list.push_back(sRegIndex);
        }
        constraint._machineReg= it->_machineReg;
        constraint._symReg = listIt->getWebReg()->num;

        newVct->push_back(constraint);
      }
    }
  }
