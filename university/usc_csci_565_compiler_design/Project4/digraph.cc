...
DiGraph& DiGraph::operator= (const DiGraph& rhs) {
  ...
  for( int i =0; i< rhs._vertexVector.size(); i++ ) {
    ...
    if( 0 != vertex_rhs_p->_firstInstr ) {
      myInstr = vertex_rhs_p->_firstInstr;
      my_simple_instr *p = 0;
      my_simple_instr *q = 0;
      ...
      while( myInstr != 0 ) {
        ...
        if(vertex_rhs_p->_firstInstr == myInstr) {
          vertex_p->_firstInstr = p;
        }else if(vertex_rhs_p->_lastInstr == myInstr) {
          vertex_p->_lastInstr = p;
        }
...
bool DiGraph::addVertex( struct basic_block *block,
	...
	if( ! isVertexExist(vertexId) ){
		Vertex *vertex = new Vertex(block, deleteBlock );
