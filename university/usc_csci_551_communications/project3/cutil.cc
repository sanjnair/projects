...
/*!
  Returns the hex representation of the Buffer
*/
string CUtil::getHexStr(const UCharBuffer &b)
{
  return CUtil::getHexStr(&(b.at(0)), b.size());
}
...
*/
bool CUtil::startsWith(
  const string &s, const string &pattern, bool cs)
{
  string s1 = (cs) ?  s : toLower(s);
  string p1 = (cs) ? pattern : toLower(pattern);

  if (s1 == p1) {
    return true;
  }
  
  string::size_type pos = s1.find(p1);
...
bool CUtil::endsWith(const string &s, const string &pattern, bool cs)
{
  string s1 = (cs) ? toLower(s) : s;
  string p1 = (cs) ? toLower(pattern) : pattern;  

  string::size_type pos = s1.rfind(p1);
  if ((string::npos != pos) && (pos == (s.size() - p1.size()))) {
    return true;
  }
  return false;
}
...
*/
string CUtil::toLower(const string &s)
{
   if (s.length() <=0) {
      return s;
   }
   ...
   string s1;
   s1.resize(s.size());
   std::transform(s.begin(), s.end(), s1.begin(), (int(*)(int)) std::tolower);
   return s1;
}
...
*/
string CUtil::toUpper(const string &s)
{
   if (s.length() <=0) {
      return s;
   }
   ...
   string s1;
   s1.resize(s.size());
   std::transform(s.begin(), s.end(), s1.begin(), (int(*)(int)) std::toupper);
   return s1;
}

/*!
  Compares two string.
  If cs is false, this method performs case insensitive comparison
*/
bool CUtil::isEqual(const string &s1, const string &s2, bool cs)
{
  if (cs) {
    return (s1 == s2);
  }
...
/*!
  Splits a string for the given separator and returns
  the list of strings .
...
*/
StringVector CUtil::split(const string &str, const string &sep)
{
  StringVector vct;
  ...
  delimIndex = str.find(sep, offset);

  while (delimIndex != string::npos) {
    vct.push_back(str.substr(offset, delimIndex - offset));
    offset += delimIndex - offset + sep.length();
    delimIndex = str.find(sep, offset);
  }
  vct.push_back(str.substr(offset));

  return vct;
}

/*!
  Returns the name value pair for the string
*/
void CUtil::getNvp(
    ...
    string &name, string &value)
{
  ...
  if (string::npos == pos) {
    string msg = string("Invalid name/value field ") + s;
    C_THROW(msg);
  }

  name = CUtil::trim(s.substr(0, pos));
  if ((pos+1) < s.length()) {
    value = CUtil::trim(s.substr(pos+1, s.length()-1));
  }

  if (name.length() <= 0) {
    C_THROW(string("Invalid name in the NVP for line ") + s);
  }
}
...
/*!
  Returns true of the time is elapsed
*/
bool CUtil::isElapsed(uint past, uint cur, uint duration)
{
  if (past > cur) {
    string msg = string("Invalid time. Current = ") + getString(cur);
    msg += string(" past time = ") + getString(past);
    C_THROW(msg);
  }
  return ((cur -past) > duration);
}
...
/*!
  Returns the path of the file given the directory and filename
*/
string CUtil::getPath(const string &dir, const string &file)
{
  string path = dir;
  
  if (! endsWith(dir, "/", true)) {
    path += "/";
  }
  path += file;
...
/*!
  Returns the filename part of the file path
*/
string CUtil::getFileName(const string &path)
{
  string name = path;
